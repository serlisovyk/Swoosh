...
description: Build, refactor, and review Swoosh authentication flows across the NestJS backend and Next.js frontend. Use when changing login, register, social login, logout, refresh-token rotation, current-user fetches, email verification, password reset flows, cookie-based auth, protected route behavior, auth-aware React Query flows, or auth-related Swagger documentation.
---
...
- Keep login, register, social login, refresh, and logout behavior centered in the auth module.
- Keep social-auth strategies, callbacks, user linking or creation, and cookie issuance centered in the auth module instead of scattering provider logic across unrelated layers.
- Under cookie auth, prefer redirecting provider callbacks to a frontend callback page that can warm `me` state before the final route transition instead of passing tokens through the URL.
- Keep guards and decorators aligned with cookie-based access token extraction.
- Prefer feature-local auth decorators for stable guard combinations such as `@Auth()`, `@GoogleAuth()`, and `@GithubAuth()` instead of repeating raw `@UseGuards(...)` in controllers.
...
- If a provider needs an extra verified-email lookup, keep that requirement explicit and keep the failure path predictable.
...
- If auth flow gains or changes social providers, update setup docs and env samples for callback URLs and provider credentials.
